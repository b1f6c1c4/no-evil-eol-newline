...
export default class LineNumberView {
  constructor(editor) {
    ...
    this.subscriptions.add(this.editor.onDidChangeSoftWrapped(toUpdate));
    this.subscriptions.add(this.editorView.onDidChangeScrollTop(this.update));
    this.subscriptions.add(atom.config.onDidChange('no-evil-eol-newline.markBufferRow', this.update));
    this.subscriptions.add(atom.config.onDidChange('no-evil-eol-newline.markScreenRow', this.update));
  ...
  update() {
    const markBufferRow = atom.config.get('no-evil-eol-newline.markBufferRow');
    const markScreenRow = atom.config.get('no-evil-eol-newline.markScreenRow');
